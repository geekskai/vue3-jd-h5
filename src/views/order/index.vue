    ...
    <section class="order-tag" ref="searchWrap">
      <list-scroll :scroll-data="categoryData" class="nav-side-wrapper" :scrollX='true'>
        <span
          ...
          @click="selectTag"
        >{{item}}</span>
        <!-- <span :class="{'active' : orderType===1}" v-for="(item,index) in categoryData" :key="index" data-type="1" @click="selectTag">全部(20)</span> -->
...
export default {
  ...
  methods: {
     setSearchWrapWidth() {
      ...
      this.$refs.searchWrap.style.width = $screenWidth - 300 + "px";
